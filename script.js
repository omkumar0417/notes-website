...
searchInput.addEventListener("input", () => {
 ...
 for (let subject in notesData) {
  for (let note of notesData[subject]) {
    const isNotePrivate = note.private === true;

    if (isNotePrivate && !isLoggedIn) {
      continue; // Skip private notes unless logged in
    }
...
function renderSidebar() {
  ...
  topicA.style.cursor = "not-allowed";
  topicA.addEventListener("click", () => {
    alert("This note is private. Please login to view it.");
  });
} else {
...
if (note.private && !isLoggedIn) {
  mainContent.innerHTML = `
    <div class="locked-note">
      <h2>🔒 Private Note</h2>
      <p>This note requires login to view.</p>
